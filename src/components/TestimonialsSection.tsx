...
export default function TestimonialsSection() {
  return (
    <section className="py-24 md:py-40 px-6 md:px-12 bg-background relative overflow-hidden">
      <motion.div
        className="absolute top-20 left-1/4 w-48 md:w-64 h-48 md:h-64 rounded-full bg-primary/5 blur-3xl pointer-events-none"
        animate={{ y: [0, -30, 0], scale: [1, 1.1, 1] }}
        transition={{ duration: 8, repeat: Infinity, ease: "easeInOut" }}
      />
      <motion.div
        className="absolute bottom-20 right-1/4 w-36 md:w-48 h-36 md:h-48 rounded-full bg-secondary/5 blur-3xl pointer-events-none"
        animate={{ y: [0, 25, 0], scale: [1, 0.9, 1] }}
        transition={{ duration: 10, repeat: Infinity, ease: "easeInOut", delay: 2 }}
      />

      <motion.div
        className="relative z-10 max-w-4xl mx-auto text-center space-y-8 md:space-y-12"
      ...
      >
        ...
        <motion.p
          className="font-headline text-2xl sm:text-3xl md:text-4xl italic text-on-background leading-relaxed"
          variants={fadeUp}
        >
