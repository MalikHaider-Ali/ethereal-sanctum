...
export default function AboutHero() {
  ...
  // Pointer-based magnetic tilt on headline
  ...
  const rotateY = useSpring(useTransform(mouseX, [-600, 600], [-5, 5]), {
    stiffness: 80,
    damping: 22,
  });
  ...
  return (
    ...
    >
      ...
      {/* Pointer-tracking ambient glow orb */}
      <motion.div
        className="absolute pointer-events-none z-[5] w-[560px] h-[560px] rounded-full -translate-x-1/2 -translate-y-1/2"
        style={{
          left: `calc(50% + ${glowX}px)`,
          top: `calc(50% + ${glowY}px)`,
          background:
            "radial-gradient(circle, rgba(230,213,180,0.08) 0%, transparent 65%)",
        }}
      />

      {/* Hero text with parallax + pointer tilt */}
      <motion.div
        className="relative z-10 text-center px-6"
        style={{ y: textY, opacity }}
      >
        <motion.span
          className="font-label text-primary uppercase tracking-[0.4em] text-sm mb-8 block"
        ...
        <motion.div
          ...
          // perspective applied via inline style
        >
          <motion.h1
            className="font-headline font-light text-6xl md:text-8xl lg:text-[clamp(72px,9vw,120px)] text-primary leading-tight tracking-tight"
      ...
      >
        ...
        <motion.div
          className="w-px h-12 bg-gradient-to-b from-primary/60 to-transparent origin-top"
